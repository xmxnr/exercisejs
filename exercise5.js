...
function firstWordOfASentence(sentence) {
	return sentence[0];
}

console.log(firstWordOfASentence('Como estas '));
...
console.log(capitalizeWord('SUMMER'));
...
function backDoorResponse(line) {
	...
	const lastLetterIndex = trimmedLine.length;

	return trimmedLine[lastLetterIndex - 1];
}
...
function backDoorResponsePolite(line) {
	const firstLetter = line[0].toUpperCase();
	const restOfWord = line.slice(1).toLowerCase();

	return firstLetter + restOfWord + ',' + ' ' + 'please';
}

console.log(backDoorResponsePolite('horse'));
